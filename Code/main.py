...
from solution import Solution, Bonus
...
def bonus():
    ...
    # section 14 - Compute Sum-Absolute-Diff distance
    tt = tic()
    sadd = bonus.sad_distance(left_image.astype(np.float64),
                                 right_image.astype(np.float64),
                                 win_size=WIN_SIZE,
                                 dsp_range=DISPARITY_RANGE)
    print(f"(Bonus 14: SAD) SADD calculation done in {toc(tt):.4f}[seconds]")

    # Construct naive disparity image
    tt = tic()
    label_map = solution.naive_labeling(sadd)
    print(f"(Bonus 14: SAD) Naive labeling done in {toc(tt):.4f}[seconds]")
    ...
    plt.imshow(label_map)
    plt.colorbar()
    plt.title('(Bonus 14: SAD) Naive Depth')

    # Smooth disparity image - Dynamic Programming
    tt = tic()
    label_smooth_dp = solution.dp_labeling(sadd, COST1, COST2)
    print(f"(Bonus 14: SAD) Dynamic Programming done in {toc(tt):.4f}[seconds]")
    ...
    plt.imshow(label_smooth_dp)
    plt.colorbar()
    plt.title('(Bonus 14: SAD) Smooth Depth - DP')
    ...
    plt.subplot(1, 3, 2)
    plt.imshow(mapped_image_smooth_dp)
    plt.title('(Bonus 14: SAD) Smooth Forward map - DP')
    plt.subplot(1, 3, 3)
    plt.imshow(right_image)
    plt.title('Right Image')
    ...
    print(f"(Bonus 14: SAD) Dynamic programming in all directions done in {toc(tt):.4f}"
          f"[seconds]")
    ...
    print(f"(Bonus 14: SAD) SGM done in {toc(tt):.4f}[seconds]")
    ...
    plt.imshow(label_smooth_sgm)
    plt.colorbar()
    plt.title('(Bonus 14: SAD) Smooth Depth - SGM')
    ...
    plt.subplot(1, 3, 2)
    plt.imshow(mapped_image_smooth_sgm)
    plt.title('(Bonus 14: SAD) Smooth Forward map - SGM')
    plt.subplot(1, 3, 3)
    plt.imshow(right_image)
    plt.title('Right Image')
    ...
    ssdd = solution.ssd_distance(left_image.astype(np.float64),
                                 ...
                                 win_size=WIN_SIZE,
                                 dsp_range=DISPARITY_RANGE)
    print(f"(Bonus 15) SSDD calculation done in {toc(tt):.4f}[seconds]")

    tt = tic()
    smooth_map = bonus.average_labeling(ssdd, WIN_SIZE)
    print(f"(Bonus 15) Averaging Smoothing done in {toc(tt):.4f}[seconds]")
    plt.figure()
    plt.subplot(1, 2, 1)
    plt.imshow(left_image)
    plt.title('Source Image')
    ...
    plt.colorbar()
    plt.title('(Bonus 15) Smooth Depth - Averaging')

    # Compute forward map of the left image to the right image.
    # Smooth disparity image - Dynamic Programming
    tt = tic()
    label_smooth_dp = bonus.dp_labeling(ssdd, COST1, COST2)
    print(f"(Bonus 15) Dynamic Programming done in {toc(tt):.4f}[seconds]")
    ...
    plt.imshow(label_smooth_dp)
    plt.colorbar()
    plt.title('(Bonus 15) Smooth Depth - DP')
    ...
    plt.subplot(1, 3, 2)
    plt.imshow(mapped_image_smooth_dp)
    plt.title('(Bonus 15) Smooth Forward map - DP')
    plt.subplot(1, 3, 3)
    plt.imshow(right_image)
    plt.title('Right Image')
    ...
    # Generate a dictionary which maps each direction to a label map:
    tt = tic()
    direction_to_vote = bonus.dp_labeling_per_direction(ssdd, COST1, COST2)
    print(f"(Bonus 15) Dynamic programming in all directions done in {toc(tt):.4f}"
          f"[seconds]")
    ...
    # Smooth disparity image - Semi-Global Mapping
    tt = tic()
    label_smooth_sgm = bonus.sgm_labeling(ssdd, COST1, COST2)
    print(f"(Bonus 15) SGM done in {toc(tt):.4f}[seconds]")
    ...
    plt.imshow(label_smooth_sgm)
    plt.colorbar()
    plt.title('(Bonus 15) Smooth Depth - SGM')
    ...
    plt.subplot(1, 3, 2)
    plt.imshow(mapped_image_smooth_sgm)
    plt.title('(Bonus 15) Smooth Forward map - SGM')
    plt.subplot(1, 3, 3)
    plt.imshow(right_image)
    plt.title('Right Image')
